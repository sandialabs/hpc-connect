...
def test_config_launch_basic():
    with hpc_connect.config.override():
        ...
        hpc_connect.config.set("launch:local_options", ["-c", "-d"])
        assert hpc_connect.config.get("launch:local_options") == ["-c", "-d"]
        hpc_connect.config.update("launch:local_options", ["-a", "-b"])
        assert hpc_connect.config.get("launch:local_options") == ["-a", "-b", "-c", "-d"]
...
def test_config_reset():
    with hpc_connect.config.override():
        ...
        hpc_connect.config.set("launch:local_options", ["-a", "-b"])
        hpc_connect.config.update("launch:local_options", ["-a", "-b"])
